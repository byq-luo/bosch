...
class mylstm(nn.Module):
  def __init__(self, hidden_dim, input_dim, output_dim):
    ...
    self.attn = nn.Linear(self.hidden_dim + self.output_dim, WINDOWWIDTH)
  ...
  def forward(self, data):
    ...
    hidden = torch.zeros((1,1,self.hidden_dim),device=torch.device('cuda'))
    input = torch.zeros((1,1,self.output_dim),device=torch.device('cuda'))
...
def train(trainSequences, testSequences):
  ...
  for epoch in range(5000):
    for i, (x, y) in enumerate(trainSequences):
      ...
      loss.backward()
      if i % 8 == 0:  # use minibatches of size 10
        optimizer.step()
        model.zero_grad()
