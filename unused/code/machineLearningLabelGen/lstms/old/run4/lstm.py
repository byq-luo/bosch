...
# TODO what happens if we use YOLO bounding boxes instead of SORT
# TODO do random data augmentation in the training loop.
...
class Model(nn.Module):
  ...
  def beamDecode(self, data:torch.Tensor):
    ...
    for i in range(NUMLABELS):
      beams.append(([output.view(1,NUMLABELS)], hidden, oneHot[i], float(output[0,0,i])))
    ...
    for i in range(WINDOWWIDTH//PREDICT_EVERY_NTH_FRAME - 1):
      ...
      for beam in beams:
        ...
        output, hidden = self.decoderStep(input, hidden, context_seq)
        for i in range(NUMLABELS):
          newBeam = (outputs + [output.view(1,NUMLABELS)], hidden, oneHot[i], beamLogProb + float(output[0,0,i]))
...
def evaluate(model, lossFunction, sequences, saveFileName):
  ...
  with open(saveFileName, 'w') as f:
    f.write(str(avgloss/i) + ' ' + str(avgacc1/i) + ' ' + str(avgacc2/i) + ' ' + str(avgiou/i) + '\n\n')
    f.writelines(outputs)

  return avgloss / i, avgacc1 / i, avgacc2 / i, avgiou / i

def checkpoint(epoch, losses, model, optimizer, lossFunction, trainSequences, testSequences):
  model.eval()
  with torch.no_grad():
    avgtrainloss, avgtrainacc, avgtrainacc2, avgtrainiou = evaluate(model, lossFunction, trainSequences, 'trainOutputs.txt')
    avgtestloss, avgtestacc, avgtestacc2, avgtestiou = evaluate(model, lossFunction, testSequences, 'testOutputs.txt')
