...
def getSequencesForFiles(files, allxs, classCounts):
  ...
  for fileI, file in enumerate(files):
    labelsFilePath = file.replace('featuresNew/', 'groundTruthLabels/').replace('_m0.pkl', '_labels.txt')
...
if __name__ == '__main__':
  ...
  if PRECOMPUTE:
    ...
    filepaths = []
    for (dirpath, dirnames, filenames) in os.walk('precomputed/featuresNew'):
